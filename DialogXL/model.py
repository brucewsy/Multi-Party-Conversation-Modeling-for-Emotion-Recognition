...
class BertERC(nn.Module):
    ...
    def __init__(self, args, num_class):
        ...
        # bert_incoder
        self.bert = BertModel.from_pretrained(args.home_dir + args.bert_model_dir)
...
class ERC_xlnet(nn.Module):
    def __init__(self, args, num_class, use_cls = True):
        ...
        if args.basemodel == 'xlnet':
            self.xlnet = XLNetModel.from_pretrained(args.home_dir + args.bert_model_dir)
        elif args.basemodel == 'xlnet_dialog':
            self.xlnet = XLNetModel_dialog.from_pretrained(args.home_dir + args.bert_model_dir)
    ...
    def forward(self, content_ids, mems, content_mask, content_lengths, speaker_ids, speaker_mask, window_mask ):
        ...
        if self.args.basemodel == 'xlnet':
            ...
            tt = self.xlnet(input_ids=content_ids, mems=mems, content_lengths = content_lengths,
                                                speaker_ids = speaker_ids, content_mask = content_mask, speaker_mask = speaker_mask,
                                                window_mask = window_mask, windowp = self.args.windowp,num_heads = self.args.num_heads)  # (B, L, D), tuple
        text_feature, new_mems, speaker_mask, window_mask,atten_matrice = tt.last_hidden_state,tt.mems,tt.speaker_mask,tt.window_mask,tt.attentions
        # print('tex_feature', text_feature.size())
        if self.use_cls:
        ...
        # print('outputs', outputs.size())
        # exit()
        # return outputs, new_mems, speaker_mask, window_mask,atten_matrice
