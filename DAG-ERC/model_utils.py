...
class GAT_dialoggcn_v2(nn.Module):
    ...
    def __init__(self, hidden_size):
        ...
        self.Wr0 = nn.Linear(hidden_size, hidden_size, bias = False)
        self.Wr1 = nn.Linear(hidden_size, hidden_size, bias = False)
        self.rel_emb = nn.Embedding(5, hidden_size)

    def forward(self, Q, K, V, adj, s_mask, r_mask):
        '''
        imformation gatherer with linear attention
        :param Q: (B, D) # query utterance
        :param K: (B, N, D) # context
        ...
        :return:
        '''
        rel_emb = self.rel_emb(r_mask) # (B, N, D)
        B = K.size()[0]
        N = K.size()[1]
        # print('Q',Q.size())
        Q = Q.unsqueeze(1).expand(-1, N, -1) # (B, N, D)；
...
class GAT_dialoggcn_v3(nn.Module):
    '''
    use linear to avoid OOM
    H_i = alpha_ij(W_rH_j)
    alpha_ij = attention(H_i, H_j)
    ...
    def __init__(self, hidden_size):
        ...
        self.hidden_size = hidden_size
        self.linear = nn.Linear(hidden_size * 2, 1)
        #self.rel_emb = nn.Embedding(10, hidden_size)
        self.Wr = nn.ModuleList([nn.Linear(hidden_size, hidden_size, bias=False) for _ in range(3)])
    ...
    def forward(self, Q, K, V, adj, s_mask):
        ...
        s_mask = s_mask.unsqueeze(-1).float()   # (B, N, 1)
        
        V_temp = self.Wr[0](V) * (s_mask[:,0,:,:])
        
        for i in range(1, 3):
            V_temp += self.Wr[i](V) * (s_mask[:,i,:,:])
        V = V_temp
