...
class DAGERC_fushion(nn.Module):
    ...
    def __init__(self, args, num_class):
        ...
        if self.args.attn_type == 'linear':
            ...
            for _ in range(args.gnn_layers):
                ...
                gats += [GAT_dialoggcn_v3(args.hidden_dim)]
            self.gather = nn.ModuleList(gats)
        ...
        # self.fcs = nn.Linear(args.hidden_dim * 2, args.hidden_dim)

        self.fc1 = nn.Linear(args.emb_dim, args.hidden_dim)

        self.nodal_att_type = args.nodal_att_type
        if self.nodal_att_type==None:
            in_dim = args.hidden_dim * (args.gnn_layers + 1) + args.emb_dim
        else:
            in_dim = args.hidden_dim * (args.gnn_layers + 1) + args.emb_dim
    ...
    def forward(self, features, adj,s_mask,s_mask_onehot,lengths):
        ...
        H0 = F.relu(self.fc1(features)) # (B, N, D)?   (B, N, H)
        # H0 = self.dropout(H0)
        H = [H0]
        weights = [[] for _ in range(B)]
        for l in range(self.args.gnn_layers):
            #C = self.grus_c[l](H[l][:,0,:]).unsqueeze(1) # (B, 1, D)?    (B, 1, H)
            #M = torch.zeros_like(C).squeeze(1) # (B, H)
            M = torch.zeros(B, H0.size(-1)).to(features)
            # P = M.unsqueeze(1) 
            P = self.grus_p[l](M, H[l][:,0,:]).unsqueeze(1)  # (B, 1, H)
            #H1 = F.relu(self.fcs[l](torch.cat((C,P) , dim = 2)))  # (B, 1, H)
            #H1 = F.relu(C+P)
            H1 = P
            for i in range(1, num_utter):
                # print(i,num_utter)
                if self.args.attn_type == 'rgcn':
                    #W, M = self.gather[l](H[l][:,i,:], H1, H1, adj[:,i,:i], s_mask[:,:,i,:i])
                    W, M = self.gather[l](H[l][:,i,:], H1, H1, adj[:,i,:i], s_mask[:,i,:i])
                    if(l==0):
                        W = W.squeeze(1).cpu().detach().numpy().tolist() # (B, N)
                        for t in range(B):
                ...
                #C = self.grus_c[l](H[l][:,i,:], M).unsqueeze(1)# (B,1,D)?  (B, 1, H)
                P = self.grus_p[l](M, H[l][:,i,:]).unsqueeze(1)   # (B, 1, H)
                # P = M.unsqueeze(1)
                #H_temp = F.relu(self.fcs[l](torch.cat((C,P) , dim = 2)))   # (B, 1, H)
                #H_temp = F.relu(C+P)
                H_temp = P
                H1 = torch.cat((H1 , H_temp), dim = 1)   # (B, N, H)
                # print('H1', H1.size())
                # print('----------------------------------------------------')
            H.append(H1)
        H.append(features)
        ...
        if self.nodal_att_type==None:
            H = torch.cat(H, dim = 2) #(B, N, l*D)?   (B, N, D + (L+1) * H)
        else:
            #H = H[-2]
            H = torch.cat(H, dim = 2)

        H = self.attentive_node_features(H,lengths,self.nodal_att_type)  # final nodal attention
        # H = self.dropout(H)
        logits = self.out_mlp(H)
